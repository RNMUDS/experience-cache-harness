...
md = Path("paper.md").read_text(encoding="utf-8")
lines = md.splitlines()
...
for l in body_lines:
    ...
    if img.match(l.strip()):
        sep.append("")
Path("_body.md").write_text("\n".join(sep), encoding="utf-8")

# --- pandoc: body.md -> latex fragment (natbib citations) ---
subprocess.run(["pandoc", "_body.md", "-t", "latex", "--natbib", "--wrap=preserve",
                "--syntax-highlighting=none", "--shift-heading-level-by=-1", "-o", "_body.tex"], check=True)
tex = Path("_body.tex").read_text(encoding="utf-8")
...
tex = sanitize(tex)
Path("_body_ieee.tex").write_text(tex, encoding="utf-8")
...
Path("paper_ieee.tex").write_text(main, encoding="utf-8")
